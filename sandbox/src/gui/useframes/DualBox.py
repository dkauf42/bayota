...
class DualBox(tk.Frame):
    def __init__(self, parent, list_items):
        ...
        list_items = sorted(list_items, key=str.lower)  # list is alphabetized
        ...
        # Copy All Button
        self.removeButton = tk.Button(self.buttonframe, text=">>>", command=self.add_all_items_to_chosen)
        self.removeButton.grid(row=0, column=0, sticky='we')
        # Copy Button
        self.copyButton = tk.Button(self.buttonframe, text=">", command=self.add_item_to_chosen)
        self.copyButton.grid(row=1, column=0, sticky='we')
        # Remove Button
        self.removeButton = tk.Button(self.buttonframe, text="<", command=self.remove_item_from_chosen)
        self.removeButton.grid(row=2, column=0, sticky='we')
        # Remove All Button
        self.removeButton = tk.Button(self.buttonframe, text="<<<", command=self.remove_all_items_from_chosen)
        self.removeButton.grid(row=3, column=0, sticky='we')
    ...
    def set_new_left_side_items(self, items):
        ...
        self.remove_all_items_from_chosen()
    ...
    def add_all_items_to_chosen(self):
        original_chosen_items = self.chosen.get(0, tk.END)
        all_items_to_add = self.listBox.get()
        list_items = sorted(original_chosen_items + all_items_to_add, key=str.lower)  # alphabetize
        self.chosen.setlist(list_items)
    ...
    def add_item_to_chosen(self):
        ...
        if selected:
            ...
            list_items = sorted(original_chosen_items + tuple(selected_minus_duplicates), key=str.lower)  # alphabetize
            self.chosen.setlist(list_items)
            # ..and remove from left-side box
    ...
    def remove_all_items_from_chosen(self):
        original_unselected_items = self.listBox.get()
        original_chosen_items = self.chosen.get(0, tk.END)
        self.chosen.clear()
        # ..and add to left-side box
