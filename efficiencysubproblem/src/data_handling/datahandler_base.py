...
class DataHandlerBase:
    def __init__(self, save2file=True, geolist=None):
        ...
        TblBmp = self.jeeves.source.TblBmp.copy()
        TblBmpGroup = self.jeeves.source.TblBmpGroup.copy()
        TblBmpLoadSourceGroup = self.jeeves.source.TblBmpLoadSourceGroup.copy()
        TblBmpType = self.jeeves.source.TblBmpLoadSourceGroup.copy()

        TblLoadSource = self.jeeves.source.TblLoadSource.copy()
        ...
        TblLandRiverSegment = self.jeeves.source.TblLandRiverSegment.copy()

        TblGeography = self.jeeves.source.TblGeography.copy()
        TblGeographyLrSeg = self.jeeves.source.TblGeographyLrSeg.copy()
        TblGeographyType = self.jeeves.source.TblGeographyType.copy()

        # Data tables for the parameter definitions
        TblBmpEfficiency = self.jeeves.source.TblBmpEfficiency.copy()
        # Target load reductions ???  (set this ourselves??)
        TblLandUsePreBmp = self.jeeves.source.TblLandUsePreBmp.copy()
        TblCostBmpLand = self.jeeves.metadata_tables.TblCostBmpLand.copy()
        Tbl2010NoActionLoads = pd.read_csv(os.path.join(get_raw_data_dir(), '2010NoActionLoads.csv'))
    ...
    def _load_set_lrsegs_from_lrseg_list(self, TblLandRiverSegment, lrsegs_list):
        ...
        totalacres = self.jeeves.lrseg.totalacres_for(lrsegnames=lrsegs_list)
