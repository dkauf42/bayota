...
def basic_solve(modelhandler, mdl, output_file_str='', fileprintlevel=4, translate_to_cast_format=False):
    ...
    if translate_to_cast_format:
        ...
        merged_df = singlelsgrpdf.loc[:, ['loadsourceshortname',
        ...
        # Add state abbreviations to the solution table
        lrsegids = jeeves.geo.lrseg.ids_from_names(merged_df['landriversegment'])
        merged_df['StateAbbreviation'] = jeeves.geo.statenames_from_lrsegids(lrsegids)

        # Rename Columns
        merged_df = merged_df.rename(index=str, columns={"landriversegment": "GeographyName"})
        merged_df = merged_df.rename(index=str, columns={"loadsourcegroup": "LoadSourceGroup"})
        merged_df = merged_df.rename(index=str, columns={"bmpshortname": "BmpShortName"})
        merged_df = merged_df.rename(index=str, columns={"acres": "Amount"})

        # Add Columns
        merged_df['AgencyCode'] = 'NONFED'
        merged_df['Unit'] = 'Acres'
        merged_df['StateUniqueIdentifier'] = ' '
