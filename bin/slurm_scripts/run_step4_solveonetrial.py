...
from bayota_util.spec_and_control_handler import notdry, read_trialcon_file, write_control_with_uniqueid
...
def main(control_file, s3_workspace_dir=None, dryrun=False, log_level='INFO') -> int:
    ...
    if notdry(dryrun, logger, f"--Dryrun-- Would run trial and save outputdf at: {notreal_notimestamp_outputdfpath}"):
        ...
        # A trialdone control ("trialdone") file is generated by adding to the trialcon file.
        control_dict['run_timestamps']['step5_done'] = datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        trialdone_control_name = write_control_with_uniqueid(control_dict=control_dict,
                                                             control_name_prefix='step5_done')
        if not not s3_workspace_dir:
            move_controlfile_to_s3(logger, get_s3_control_dir(), s3ops,
                                   controlfile_name=trialdone_control_name, no_s3=False, )
