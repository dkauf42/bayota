...
class DataLoader:
    def __init__(self, save2file=True, geoscale='lrseg', geolist=None):
        ...
        self.load_sets(self.save2file, TblLandRiverSegment, TblBmp, TblBmpType, TblBmpEfficiency,
                       ...
                       singlelsgrpdf, baseconditionid, geoscale=geoscale, geolist=geolist)
    ...
    def load_sets(self, save2file, TblLandRiverSegment, TblBmp, TblBmpType, TblBmpEfficiency,
        ...
        self.PLTNTS = p_list
        if save2file:
            pd.DataFrame(p_list, columns=['PLTNTS']).to_csv('data_PLTNTS.tab', sep=' ', index=False)
        ...
        lrsegids = jeeves.lrseg.lrsegids_from(lrsegnames=lrsegs_list).lrsegid.tolist()
        if not lrsegs_list:
            print('uh oh. No lrsegs were specified. ...Proceeding with a single default lrseg.')
            lrsegs_list = ['N42071SL2_2410_2700']
            lrsegids = TblLandRiverSegment[TblLandRiverSegment['landriversegment'] == lrsegs_list[0]].lrsegid.tolist()
...
# dl = DataLoader(save2file=False, geoscale='county', geolist=['Anne Arundel, MD'])
